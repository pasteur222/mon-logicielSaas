...
const BusinessPayment: React.FC = () => {
  ...
  useEffect(() => {
    if (planId) {
      const foundPlan = BUSINESS_PLANS.find(p => p.id === planId);
      if (foundPlan) {
        setPlan(foundPlan);
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!phoneNumber || !email || !companyName || !password) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">Plan non trouvé</h2>
          <p className="text-gray-600 mb-6">Le plan sélectionné n'existe pas.</p>
          <button
            onClick={() => navigate('/')}
